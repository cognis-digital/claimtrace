"""Native cognis-connect emit for claimtrace — forward findings to any platform.

Maps claimtrace's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    claimtrace ... --format json | claimtrace-emit --to stix
    claimtrace-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('source', '') + ' - ' + rec.get('variant', '')
