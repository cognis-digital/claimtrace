...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CL-EARLY-001', 'severity': 'info', 'weight': 1.0, 'title': 'EARLIEST_SOURCE', 'when': {'earliest': True}, 'remediation': 'Use as anchor for provenance graph.'}, {'id': 'CL-COORD-001', 'severity': 'high', 'weight': 2.5, 'title': 'COORDINATED_AMPLIFICATION', 'when': {'coordinated': True}, 'remediation': 'Document; cross-platform escalation likely warranted.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="info-integrity",
                    metadata=item,
                ))
